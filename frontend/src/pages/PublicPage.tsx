    ...
    <div className={`public-shell ${isDarkMode ? "public-shell--dark" : "public-shell--light"}`}>
      ...
      <main className="public-main">
        ...
        <section className="public-features">
          {FEATURES.map((f) => (
            <div key={f.title} className="public-feature">
              <span className="public-feature__icon">{f.icon}</span>
              <h2 className="public-feature__title">{f.title}</h2>
              <p className="public-feature__desc">{f.desc}</p>
            </div>
          ))}
        </section>
      </main>
...
const FEATURES = [
  {
    ...
    desc: "Instantly spin up reproducible JupyterLab, Dask, or GPU environments without managing infrastructure.",
  },
  {
    icon: "🔒",
    title: "Secure by default",
    desc: "Role-based access control and SSO-backed auth powered by Keycloak, so your data stays safe.",
  },
  {
    icon: "🤝",
    title: "Built for collaboration",
    desc: "Share notebooks, dashboards, and pipelines with your team in a single managed workspace.",
  },
];
