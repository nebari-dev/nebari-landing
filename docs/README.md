...
- `dev-quickstart.md` - Frontend Dev Quickstart (docker-compose + MSW loop).
- `design/landing-page.md` - Landing Page Design (architecture doc).
- `maintainers/release-checklist.md` - Release Checklist.

Still to migrate:

- `docs/api.md` - auto-generated by `make docs` (swag -> apicard -> gendocs). The pipeline writes to `docs/api.md` at the repo root; migration means pointing the generator at `src/content/docs/api.md` (or keeping a symlink) and updating the `openapi-drift` workflow's path filter. Follow-up PR.
- `docs/design/ws-per-client-access-filter.md` - lives on PR #124's branch; will land here once that PR merges.
